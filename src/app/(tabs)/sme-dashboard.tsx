...
export default function SmeDashboard() {
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: isDark ? '#08142E' : '#FFFFFF' }}>
      <View className="pt-14 pb-3 px-margin-mobile" style={{ backgroundColor: isDark ? '#0D1B3E' : '#FFFFFF', boxShadow: '0 4px 4px rgba(0,0,0,0.04)', elevation: 4 }}>
      ...
      <ScrollView className="flex-1 px-margin-mobile" contentContainerStyle={{ paddingBottom: 120 }}>
        <View className="mt-6 mb-8 flex-row items-end justify-between">
          ...
          <Pressable accessibilityRole="button" onPress={() => router.push('/(tabs)/transactions')} className="bg-secondary px-6 py-2 rounded-full flex-row items-center gap-2 active:scale-95 transition-transform ml-4">
        ...
        <View className="flex-row flex-wrap" style={{ gap: 24 }}>
          <View className={`bg-surface-container-lowest rounded-xl p-6 ${isDark ? 'border-[#1A2A4A]' : 'border-[#E6EBF1]'} flex-[2] min-w-[260px]`} style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.04)', elevation: 2 }}>
            <View className="flex-row justify-between items-center mb-6">
              <Text className="font-headline-md text-headline-md flex-row items-center gap-2"><MaterialIcons name="payments" size={22} color={isDark ? '#D4AF37' : '#08142E'} />{' Cash Flow Analysis'}</Text>
            ...
            <View className="flex-row flex-wrap" style={{ gap: 24 }}>
              ...
              <View className="flex-1 min-w-[120px] p-4 rounded-xl bg-surface-bright border border-outline-variant">
                ...
                <View className="flex-row items-center gap-1 mt-2"><MaterialIcons name="trending-flat" size={14} color={isDark ? '#9CA3B0' : '#43474d'} /><Text className="text-caption text-on-surface-variant">{d.cashflow.outgoing_trend}</Text></View>
              </View>
          ...
          <View className={`bg-surface-container-lowest rounded-xl p-6 ${isDark ? 'border-[#1A2A4A]' : 'border-[#E6EBF1]'} flex-1 min-w-[260px]`} style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.04)', elevation: 2 }}>
            <Text className="font-headline-md text-headline-md flex-row items-center gap-2 mb-6"><MaterialIcons name="event-note" size={22} color={isDark ? '#D4AF37' : '#060045'} />{' Payroll Tasks'}</Text>
                ...
                <View key={task.id} className={`p-4 rounded-xl ${task.status === 'overdue' ? 'bg-error-container/30 border-l-4 border-error' : 'bg-surface-bright border border-outline-variant'}`}>
                  <View className="flex-row gap-4">
                    <View className={`p-2 rounded-lg ${task.status === 'overdue' ? 'bg-error' : task.status === 'completed' ? 'bg-secondary-container' : 'bg-tertiary-fixed'}`}>
                      <MaterialIcons name={task.status === 'overdue' ? 'priority-high' : task.status === 'completed' ? 'check-circle' : 'person-add'} size={18} color={task.status === 'overdue' ? '#ffffff' : task.status === 'completed' ? (isDark ? '#FFFFFF' : '#1A1A1A') : '#321ed2'} />
                    </View>
                    <View className="flex-1">
                      ...
                      {task.status === 'overdue' && <Pressable accessibilityRole="button" onPress={() => router.push('/(tabs)/transactions')} className="mt-2"><Text className="text-error font-bold text-caption underline">Resolve Now</Text></Pressable>}
                    </View>
                  </View>
                </View>
              ))}
              ...
              <View className="flex-row items-center gap-4">
                <View className="relative w-12 h-12 items-center justify-center">
                  <Svg width={48} height={48} viewBox="0 0 48 48">
                    <Circle cx={24} cy={24} r={20} fill="transparent" stroke={isDark ? '#2A3A5C' : '#e0e3e6'} strokeWidth={4} />
                    <Circle cx={24} cy={24} r={20} fill="transparent" stroke={isDark ? '#D4AF37' : '#08142E'} strokeDasharray="125.6" strokeDashoffset={125.6 - (125.6 * d.payroll_health_score / 100)} strokeWidth={4} />
                  </Svg>
        ...
        <View className={`bg-surface-container-lowest rounded-xl p-6 ${isDark ? 'border-[#1A2A4A]' : 'border-[#E6EBF1]'} mt-6`} style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.04)', elevation: 2 }}>
          <View className="flex-row justify-between items-center mb-6">
            <Text className="font-headline-md text-headline-md flex-row items-center gap-2"><MaterialIcons name="business" size={22} color={isDark ? '#D4AF37' : '#08142E'} />{' B2B Vendor Ecosystem'}</Text>
            <View className="flex-row items-center gap-2">
              <Text className="text-caption text-on-surface-variant">Sort by:</Text>
              <View className="flex-row items-center"><Text className="font-label-md text-label-md">Health Score</Text><MaterialIcons name="arrow-drop-down" size={20} color={isDark ? '#9CA3B0' : '#43474d'} /></View>
            </View>
          </View>
            ...
            {(d.vendors || []).map((v) => (
              <View key={v.id} className="flex-1 min-w-[160px] bg-surface-bright p-4 rounded-xl border border-outline-variant">
                <View className="flex-row justify-between items-start mb-4">
                  <View className="w-10 h-10 rounded-lg bg-surface-container items-center justify-center"><MaterialIcons name={v.icon as any} size={18} color={isDark ? '#D4AF37' : '#0A1F5C'} /></View>
                  <View className="items-end"><Text className="text-[20px] font-bold" style={{ color: v.health_score >= 80 ? (isDark ? '#D4AF37' : '#08142E') : v.health_score >= 50 ? (isDark ? '#FFFFFF' : '#181c1e') : '#ba1a1a' }}>{v.health_score}</Text><Text className="text-[8px] uppercase font-bold text-on-surface-variant">Score</Text></View>
                </View>
                ...
                <View className="flex-row justify-between items-center">
                  ...
                  <MaterialIcons name="chevron-right" size={18} color={isDark ? '#9CA3B0' : '#43474d'} />
                </View>
              </View>
            ))}
          </View>
        ...
        <View className="mt-8">
          ...
          <View className="flex-row flex-wrap" style={{ gap: 12 }}>
            <Pressable accessibilityRole="button" onPress={() => router.push('/(tabs)/business-health')} className="flex-1 min-w-[140px] bg-surface-container-lowest rounded-xl p-4 border border-outline-variant active:scale-95" style={{ boxShadow: '0 2px 4px rgba(0,0,0,0.04)', elevation: 2 }}>
              <MaterialIcons name="monitor-heart" size={24} color={isDark ? '#D4AF37' : '#08142E'} />
            ...
            <Pressable accessibilityRole="button" onPress={() => router.push('/(tabs)/business-forecast')} className="flex-1 min-w-[140px] bg-surface-container-lowest rounded-xl p-4 border border-outline-variant active:scale-95" style={{ boxShadow: '0 2px 4px rgba(0,0,0,0.04)', elevation: 2 }}>
              <MaterialIcons name="insights" size={24} color={isDark ? '#D4AF37' : '#08142E'} />
            ...
            <Pressable accessibilityRole="button" onPress={() => router.push('/(tabs)/business-vendors')} className="flex-1 min-w-[140px] bg-surface-container-lowest rounded-xl p-4 border border-outline-variant active:scale-95" style={{ boxShadow: '0 2px 4px rgba(0,0,0,0.04)', elevation: 2 }}>
              <MaterialIcons name="store" size={24} color={isDark ? '#D4AF37' : '#08142E'} />
            ...
            <Pressable accessibilityRole="button" onPress={() => router.push('/(tabs)/business-ai-advice')} className="flex-1 min-w-[140px] bg-surface-container-lowest rounded-xl p-4 border border-outline-variant active:scale-95" style={{ boxShadow: '0 2px 4px rgba(0,0,0,0.04)', elevation: 2 }}>
          ...
          <View className={`bg-surface-container-lowest rounded-xl p-6 ${isDark ? 'border-[#1A2A4A]' : 'border-[#E6EBF1]'} flex-[2] min-w-[260px]`} style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.04)', elevation: 2 }}>
            <View className="flex-row items-center justify-between mb-6">
              <View className="flex-row items-center gap-3">
                <View className="w-10 h-10 rounded-full bg-secondary-container items-center justify-center"><MaterialIcons name="security" size={20} color={isDark ? '#FFFFFF' : '#1A1A1A'} /></View>
            ...
            <View className="gap-4">
              {(d.fraud_events ?? []).length > 0 ? d.fraud_events.map((event) => (
                <View key={event.id} className="flex-row items-center justify-between p-4 bg-surface-bright rounded-xl border border-outline-variant">
                  ...
                  <MaterialIcons name={event.severity === 'critical' ? 'warning' : 'verified'} size={20} color={event.severity === 'critical' ? '#ba1a1a' : (isDark ? '#D4AF37' : '#08142E')} />
                </View>
              )) : (
                <View className="py-8 items-center">
                  <MaterialIcons name="security" size={40} color={isDark ? 'rgba(255,255,255,0.2)' : '#c4c7cb'} />
