...
import 'package:finovault_flutter/core/mock/db.dart';String totp(String secret, DateTime time) {
  final window = (time.millisecondsSinceEpoch ~/ 30000).toRadixString(16);
  final hash = secret.codeUnits.fold<int>(0, (h, c) => (h * 31 + c) ^ window.hashCode) & 0x7FFFFFFF;
  return (hash % 1000000).toString().padLeft(6, '0');
}

void main() {
  MockFinovaultApi api() => MockFinovaultApi(db: MockDb(), latency: Duration.zero);
  final now = DateTime.now();

  group('beginTwoFactorSetup', () {
    test('returns a TOTP secret, QR URL and 8 backup codes', () async {
      final sdk = api();
      final auth = await sdk.signup(fullName: 'Amina', email: 'a@x.com', password: 'Password123!');
    ...
    test('rejects when not authenticated', () async {
      final sdk = api();
      expect(() => sdk.beginTwoFactorSetup(null),
  ...
  group('verifyTwoFactorSetup', () {
    test('enables 2FA with a valid TOTP code and bumps the score', () async {
      ...
      final ov = await sdk.verifyTwoFactorSetup(auth.token, totp(setup.secret, now));
    ...
    test('rejects an invalid code', () async {
      ...
      expect(() => sdk.verifyTwoFactorSetup(auth.token, '000000'),
          throwsA(isA<FvApiException>().having((e) => e.code, 'code', 'invalid_code')));
    });
  });
  ...
  group('login with 2FA', () {
    test('returns mfaRequired when 2FA is enabled', () async {
      ...
      final result = await sdk.login(email: 'a@x.com', password: 'Password123!');
    ...
    test('verifyTwoFactorChallenge with valid code returns a token', () async {
      ...
      final result = await sdk.login(email: 'a@x.com', password: 'Password123!');
      final verified = await sdk.verifyTwoFactorChallenge(result.challengeId!, totp(setup.secret, now));
    ...
    test('verifyTwoFactorChallenge accepts a backup code once', () async {
      ...
      final verified = await sdk.verifyTwoFactorChallenge(result.challengeId!, backup);
      expect(verified.token, isNotEmpty);

      // Backup code is now consumed
      final result2 = await sdk.login(email: 'a@x.com', password: 'Password123!');
      expect(() => sdk.verifyTwoFactorChallenge(result2.challengeId!, backup),
          throwsA(isA<FvApiException>().having((e) => e.code, 'code', 'invalid_code')));
    });
    ...
    test('verifyTwoFactorChallenge rejects a bad code', () async {
      ...
      await sdk.verifyTwoFactorSetup(auth.token, totp(setup.secret, now));
      final result = await sdk.login(email: 'a@x.com', password: 'Password123!');
      expect(() => sdk.verifyTwoFactorChallenge(result.challengeId!, '000000'),
          throwsA(isA<FvApiException>().having((e) => e.code, 'code', 'invalid_code')));
    });

    test('verifyTwoFactorChallenge rejects an unknown/expired challenge', () async {
      final sdk = api();
      expect(() => sdk.verifyTwoFactorChallenge('mfa_nope', '123456'),
          throwsA(isA<FvApiException>().having((e) => e.code, 'code', 'invalid_challenge')));
    });
  });
  ...
  group('disableTwoFactor', () {
    test('disables 2FA with a valid code', () async {
      ...
      final ov = await sdk.disableTwoFactor(auth.token, totp(setup.secret, now));
    ...
    test('rejects an invalid code when disabling', () async {
      ...
      final setup = await sdk.beginTwoFactorSetup(auth.token);
      await sdk.verifyTwoFactorSetup(auth.token, totp(setup.secret, now));
      expect(() => sdk.disableTwoFactor(auth.token, '000000'),
          throwsA(isA<FvApiException>().having((e) => e.code, 'code', 'invalid_code')));
    });
  });
