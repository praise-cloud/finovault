  ...
  void _contribute(BuildContext context) {
    ...
    if (accounts.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Link an account first to contribute.')),
      );
      return;
    }
                  ...
                  items: accounts
                      .map(
                        (a) => DropdownMenuItem(
                          value: a.id,
                          child: Text(
                            '${a.name} (${FvFormat.formatMoney(a.balance)})',
                          ),
                        ),
                      )
                      .toList(),
  ...
  Widget build(BuildContext context) {
    ...
    return ScreenPage(
      ...
      actions: [
        if (plan.value != null)
          IconButton(
            icon: Icon(Icons.edit_outlined, color: context.fvPrimary),
            onPressed: () => pushScreen(context, const PensionSetupScreen()),
          ),
      ],
      child: plan.when(
                ...
                children: [
                  FvCard(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'in ${projection.yearsToRetirement} years (today’s money)',
                          style: TextStyle(
                            fontSize: 13,
                            color: context.fvTextSecondary,
                          ),
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _PotCard(
                          title: 'Short-term pot',
                          current: p.currentShortPot,
                          target: p.shortPotTarget,
                        ),
                      ),
                      ...
                      Expanded(
                        child: _PotCard(
                          title: 'Long-term pot',
                          current: p.currentLongPot,
                          target: p.longPotTarget,
                        ),
                      ),
                    ],
                  ),
                    ...
                    child: Column(
                      ...
                      children: [
                        _Row(
                          label: 'Contribution',
                          value:
                              '${p.frequency.name} · ${FvFormat.formatMoney(p.contributionAmount)}',
                        ),
                        _Row(
                          label: 'Assumed return',
                          value: '${p.assumedReturnPct.toStringAsFixed(0)}%',
                        ),
                        _Row(
                          label: 'Inflation',
                          value: '${p.inflationPct.toStringAsFixed(0)}%',
                        ),
                        _Row(
                          label: 'Auto-debit',
                          value: p.autoDebit ? 'On' : 'Off',
                        ),
                      ],
                    ),
                  ),
                            ...
                            children: [
                              for (final c in list.take(8))
                                ListTile(
                                  ...
                                  title: Text(
                                    '${c.pot == 'short' ? 'Short-term' : 'Long-term'} pot',
                                  ),
...
class _Row extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(
      ...
      children: [
        Flexible(
          child: Text(label, style: TextStyle(color: context.fvTextSecondary)),
        ),
